...
import { Facebook, Twitter, Linkedin, Link2, ArrowLeft, Bookmark } from "lucide-react";
        ...
        <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 -mt-32 relative z-10">
          ...
          <div className="md:hidden mb-12 pb-12 border-b border-border">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={handleBookmark}
                className={`flex-1 py-3 rounded-full border transition-all flex items-center justify-center gap-2 ${
                  bookmarked 
                    ? "border-primary bg-primary/10" 
                    : "border-border hover:border-primary hover:bg-muted"
                }`}
          ...
          <div className="mb-16 rounded-2xl bg-card p-8 md:p-12 text-center">
            ...
            {isSubscribed ? (
              <div className="text-center py-4">
                <p className="text-primary font-medium">You're already subscribed! ✓</p>
                <p className="text-sm text-muted-foreground mt-1">Check your inbox every Sunday.</p>
              </div>
            ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedArticles.map((relatedArticle, index) => (
                <div key={relatedArticle.id} className={`animate-slide-up stagger-${Math.min(index + 1, 3)}`}>
                  <ArticleCard {...relatedArticle} variant="standard" />
                </div>
              ))}
            </div>
          </div>
